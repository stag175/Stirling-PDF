...
export default function AdminAdvancedSection() {
  ...
  useEffect(() => {
    ...
    const fetchTessdataLanguages = async () => {
      ...
      try {
        ...
        console.error(
          "[AdminAdvancedSection] Failed to load tessdata languages",
          error,
        );
    ...
    for (let attempt = 0; attempt < retries; attempt++) {
      try {
        ...
        if (attempt === retries - 1) {
          console.error(
            "[AdminAdvancedSection] Retry refresh tessdata failed",
            err,
          );
          return;
        }
  ...
  const handleDownloadTessdataLanguages = async () => {
    ...
    try {
      ...
      console.error(
        "[AdminAdvancedSection] Download tessdata languages failed",
        error,
      );
      ...
      if (status === 403) {
        console.warn(
          "[AdminAdvancedSection] Tessdata directory not writable, falling back to manual download:",
          serverMessage,
        );
          ...
          <Stack gap="md">
            ...
            >
              ...
              <Group gap="xs">
                ...
                />
                <PendingBadge
                  show={isFieldPending("enableAlphaFunctionality")}
                />
              </Group>
            </div>
            ...
            <div>
              ...
              <Stack gap="xs" mt="sm">
                <MultiSelect
                  ...
                  searchable
                  disabled={
                    !loginEnabled || remoteTessdataLanguages.length === 0
                  }
          ...
          <Stack gap="md">
            ...
            <div>
              <NumberInput
                ...
                value={
                  settings.tempFileManagement?.cleanupIntervalMinutes ?? 30
                }
                onChange={(value) =>
                  setSettings({
                    ...settings,
            ...
            >
              ...
              <Group gap="xs">
                <Switch
                  checked={
                    settings.tempFileManagement?.cleanupSystemTemp ?? false
                  }
                  onChange={(e) => {
                    if (!loginEnabled) return;
                    setSettings({
