...
-- ---------------------------------------------------------------------------------------------
-- 11. Default pricing policy seed. Idempotent — only inserts when no default row exists yet.
--     Units sized so a typical 25-page / 5 MiB document is 1 unit; tweak via admin once Stripe
--     Prices are wired up in PR-C5.
-- ---------------------------------------------------------------------------------------------
...
SELECT
    ...
    1, 1000, TRUE,
    'V1 default seeded by V11 migration. Tune via admin once Stripe Prices are configured.',
    'system'
WHERE NOT EXISTS (
    SELECT 1 FROM pricing_policy WHERE is_default = TRUE
);

-- Seed step limits for the default policy across every JobSource. Indirectly references the row
-- inserted above via the unique partial index on is_default=TRUE.
